...
export class RegisterComponent implements OnInit {
  ...
  fechaFormateadaMaxDate = this.maxDate.toLocaleDateString(
    'en-EN',
    this.opciones
  );
  fechaObj = new Date(this.fechaFormateada.split('-').reverse().join('-'));
  fechaObjMax = new Date(
    this.fechaFormateadaMaxDate.split('-').reverse().join('-')
  );
  ...
  ngOnInit(): void {
    ...
    this.registerForm.get('userRol')?.valueChanges.subscribe(rolValue => {
      this.disabledField(rolValue);
    });

    this.registerForm.get('codeCountry')?.valueChanges.subscribe(country => {
  ...
  validateUserRol(): boolean {
    if (
      this.registerForm.get('userRol')?.touched &&
      this.registerForm.get('userRol')?.errors?.['required']
    ) {
      return true;
    } else return false;
  }
  ...
  validateNameRequired(): boolean {
    if (
      ...
      this.registerForm.get('name')?.errors?.['required']
    ) {
      return true;
    } else return false;
  }
  ...
  validateemailMinLength(): boolean {
    if (
      this.registerForm.get('email')?.touched &&
      this.registerForm.get('email')?.errors?.['minlength']
    ) {
      return true;
    } else return false;
  }
  ...
  validatePassword(): boolean {
    if (
      this.registerForm.get('password')?.touched &&
      this.registerForm.get('password')?.errors?.['pattern']
    ) {
      return true;
    } else return false;
  }
  ...
  validateDateRequiered(): boolean {
    if (
      ...
      this.registerForm.get('birthday')?.errors?.['required']
    ) {
      return true;
    } else return false;
  }
  ...
  validateSexoRequiered(): boolean {
    if (
      this.registerForm.controls.sexo.invalid &&
      (this.registerForm.controls.sexo.dirty ||
        this.registerForm.controls.sexo.touched)
    ) {
      return true;
    } else return false;
  }
  ...
  validateCodeRequiered(): boolean {
    if (
      this.registerForm.get('codeCountry')?.touched &&
      this.registerForm.get('codeCountry')?.errors?.['required']
    ) {
      return true;
    } else return false;
  }
  ...
  validatePhoneRequiered(): boolean {
    if (
      ...
      this.registerForm.get('phone')?.errors?.['required']
    ) {
      return true;
    } else return false;
  }
  ...
  validatePhoneFormat(): boolean {
    if (
      this.registerForm.get('phone')?.touched &&
      this.registerForm.get('phone')?.errors?.['pattern']
    ) {
      return true;
    } else return false;
  }
  ...
  validateAcceptRequiered(): boolean {
    if (
      this.registerForm.get('accept')?.touched &&
      this.registerForm.get('accept')?.errors?.['required']
    ) {
      return true;
    } else return false;
  }
  ...
  registerUser() {
    ...
    this.bymiaservice.registerNewUser(this.newUser).subscribe(
      resp => {
        // console.log(resp);
  ...
  get userRol() {
    return this.registerForm.get('userRol');
  }
}
